...
// ─── Models ───────────────────────────────────────────────────────────────────
const IMAGE_MODELS = {
  "fal-ai/flux/schnell":       { name: "Flux Schnell",  desc: "Schnell & günstig",       icon: "⚡" },
  "fal-ai/flux-pro":           { name: "Flux Pro",      desc: "Hohe Qualität",            icon: "✦" },
  "fal-ai/flux-pro/v1.1-ultra":{ name: "Flux Ultra",    desc: "Beste Qualität",           icon: "◈" },
};

const VIDEO_MODELS = {
  "fal-ai/kling-video/v2.1/standard/image-to-video": { name: "Kling 2.1",     desc: "Realistisch, stabil",      icon: "▶" },
  "fal-ai/kling-video/v1.6/pro/image-to-video":      { name: "Kling 1.6 Pro", desc: "Bewährt, präzise",         icon: "▶" },
  "fal-ai/minimax/video-01-live":                     { name: "Hailuo",        desc: "Schnell, günstig",         icon: "▶" },
  "fal-ai/wan/v2.1/image-to-video":                  { name: "Wan 2.1",       desc: "Open Source, günstig",     icon: "▶" },
};

const VIDEO_TEXT_MODELS = {
  "fal-ai/kling-video/v2.1/standard/text-to-video":  { name: "Kling 2.1 (Text)", desc: "Kein Keyframe nötig",  icon: "▶" },
  "fal-ai/minimax/video-01":                          { name: "Hailuo (Text)",    desc: "Schnell",              icon: "▶" },
};
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        // Priority 1: ?p= encoded project data (from Claude artifact)
        if (urlParam) {
          try {
            const decoded = JSON.parse(decodeURIComponent(escape(atob(urlParam))));
            setSelected({ ...decoded, _brand: { name: "", color: "#7F77DD" } });
            setLoading(false);
            return;
          } catch(e) {
            setError("Projekt-Link konnte nicht geladen werden. Bitte neuen Link generieren.");
            setLoading(false);
            return;
          }
        }
        // Priority 2: ?project= Supabase ID (legacy)
        if (urlProjectId) {
          ...
          // Show project list from Supabase
          const { data } = await supabase
            .from("projects")
            .select("id, name, content_type, updated_at, brand_id, steps_done")
            .order("updated_at", { ascending: false });
          setProjects(data || []);
        }
      } catch(e) {
        setError("Keine Projekte gefunden. Öffne ein Projekt über das Claude Artifact.");
      }
      setLoading(false);
    })();
  }, []);
...
function ProjectList({ projects, onSelect }) {
  const CONTENT_COLORS = { product:"#7F77DD", ugc:"#1D9E75", talkinghead:"#BA7517", impossible:"#D85A30" };
  const CONTENT_LABELS = { product:"Product Ad", ugc:"UGC Style", talkinghead:"Talking Head", impossible:"AI Creative" };

  return (
    <div style={{ maxWidth:640, margin:"0 auto", padding:"32px 24px", fontFamily:"var(--font-sans,-apple-system,sans-serif)" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
        {projects.map(p => {
          const col = CONTENT_COLORS[p.content_type] || "#7F77DD";
          return (
            <div key={p.id} onClick={() => onSelect(p)} style={{ border:"0.5px solid #efefed", borderRadius:10, padding:"12px 14px", cursor:"pointer", display:"flex", alignItems:"center", gap:12, transition:"border-color .15s" }}
              ...
              <div style={{ flex:1 }}>
                <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:4 }}>
                  ...
                  <span style={{ fontSize:9, padding:"1px 6px", borderRadius:8, background:`${col}18`, color:col, fontFamily:"monospace" }}>{CONTENT_LABELS[p.content_type] || p.content_type}</span>
                </div>
...
function GenerationDashboard({ project, onBack }) {
  ...
  const [jobs, setJobs] = useState({}); // { "sceneId-type": { status, error } }
  ...
  const generateImage = async (scene) => {
    ...
    try {
      ...
      setJob(key, { status:"error", error: e.message });
    }
  };
  ...
  return (
    <div style={{ maxWidth:800, margin:"0 auto", padding:"24px 20px", fontFamily:"var(--font-sans,-apple-system,sans-serif)" }}>
      {/* Header */}
      <div style={{ display:"flex", alignItems:"flex-start", gap:12, marginBottom:20, paddingBottom:16, borderBottom:"0.5px solid #efefed" }}>
        ...
        <div style={{ flex:1 }}>
          ...
          {project.productData?.analyzed?.productName && (
            <div style={{ fontSize:12, color:"#5c5c5a", marginTop:2 }}>{project.productData.analyzed.productName}</div>
          )}
        </div>
        <button onClick={generateAll} style={{ padding:"9px 18px", borderRadius:9, border:"none", background:brand.color||"#7F77DD", color:"#fff", fontSize:13, fontWeight:500, cursor:"pointer", flexShrink:0, whiteSpace:"nowrap" }}>
          ⚡ Alle generieren
        </button>
      </div>
      ...
      <div style={{ display:"flex", gap:12, marginBottom:20, flexWrap:"wrap" }}>
        ...
        <ModelPicker label="Video-Modell" value={vidModel} onChange={setVidModel} options={{ ...VIDEO_MODELS, ...VIDEO_TEXT_MODELS }} col="#7F77DD" />
      </div>

      {scenes.length === 0 && (
        <div style={{ border:"1.5px dashed #e5e5e3", borderRadius:12, padding:40, textAlign:"center", color:"#9b9b99", fontSize:13 }}>
          Keine Szenen gefunden. Bitte erst Szenen im Claude Artifact generieren.
        </div>
      )}
          ...
          <div key={scene.id} style={{ border:"0.5px solid #efefed", borderRadius:12, overflow:"hidden", marginBottom:12 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:0 }}>
              ...
              <div style={{ padding:"12px 14px", borderRight:"0.5px solid #efefed" }}>
                <div style={{ fontSize:10, fontFamily:"monospace", letterSpacing:".06em", color:"#9b9b99", marginBottom:8 }}>KEYFRAME</div>
                {kf?.startFrame && (
                  <div style={{ fontFamily:"monospace", fontSize:10, background:"#f7f7f6", borderRadius:6, padding:"6px 8px", lineHeight:1.6, color:"#5c5c5a", marginBottom:8, wordBreak:"break-word" }}>
                    {kf.startFrame}
                  </div>
                )}
                {imgResult ? (
                  <div>
                    ...
                    <div style={{ display:"flex", gap:5 }}>
                      <a href={imgResult} download={`szene-${scene.id}-keyframe.jpg`} target="_blank" rel="noreferrer" style={{ flex:1, fontSize:11, padding:"5px 8px", borderRadius:6, border:"0.5px solid #d4d4d2", background:"transparent", color:"#5c5c5a", textAlign:"center", textDecoration:"none", cursor:"pointer" }}>↓ Download</a>
              ...
              <div style={{ padding:"12px 14px" }}>
                <div style={{ fontSize:10, fontFamily:"monospace", letterSpacing:".06em", color:"#9b9b99", marginBottom:8 }}>VIDEO</div>
                {vp?.prompt && (
                  <div style={{ fontFamily:"monospace", fontSize:10, background:"#f7f7f6", borderRadius:6, padding:"6px 8px", lineHeight:1.6, color:"#5c5c5a", marginBottom:8, wordBreak:"break-word" }}>
                    {vp.prompt}
                  </div>
                ...
                {vidResult ? (
                  <div>
                    ...
                    <div style={{ display:"flex", gap:5 }}>
                      <a href={vidResult} download={`szene-${scene.id}-video.mp4`} target="_blank" rel="noreferrer" style={{ flex:1, fontSize:11, padding:"5px 8px", borderRadius:6, border:"0.5px solid #d4d4d2", background:"transparent", color:"#5c5c5a", textAlign:"center", textDecoration:"none", cursor:"pointer" }}>↓ Download</a>
                  ...
                  <GenButton onClick={()=>generateVideo(scene)} job={vidJ} label="Video generieren" col="#7F77DD"
                    note={!gens[scene.id]?.image ? "Erst Keyframe generieren für beste Ergebnisse" : null} />
                )}
              </div>
            </div>
          </div>
...
function ModelPicker({ label, value, onChange, options, col }) {
  return (
    <div style={{ flex:1, minWidth:200 }}>
      <div style={{ fontSize:10, fontFamily:"monospace", letterSpacing:".06em", color:"#9b9b99", marginBottom:6 }}>{label.toUpperCase()}</div>
      <div style={{ display:"flex", flexWrap:"wrap", gap:4 }}>
        {Object.entries(options).map(([id, m]) => (
          <button key={id} onClick={()=>onChange(id)} style={{
            fontSize:11, padding:"4px 9px", borderRadius:6, cursor:"pointer",
            fontFamily:"-apple-system,sans-serif",
            border:`0.5px solid ${value===id?col:"#e5e5e3"}`,
            background:value===id?`${col}15`:"transparent",
            color:value===id?col:"#5c5c5a", fontWeight:value===id?500:400,
          }}>{m.name}</button>
        ))}
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <button onClick={onClick} disabled={isLoading} style={{
        ...
        background: isError ? "#FAECE7" : isLoading ? "#f0f0f0" : col,
        color: isError ? "#D85A30" : isLoading ? "#9b9b99" : "#fff",
        fontSize:12, fontWeight:500, cursor:isLoading?"default":"pointer",
